...
const resolvers = {
  Query: {
    ...
    obtenerProducto: async (_, { id }) => {
      ...
      const producto = Producto.findOne({ _id: id });
    ...
    obtenerPedidosVendedor: async (_, {}, ctx) => {
      try {
        const pedidos = Pedido.find({ vendedor: ctx.usuario.id }).populate(
          'cliente'
        );
        return pedidos;
      } catch (error) {
  ...
  Mutation: {
    ...
    actualizarProducto: async (_, { id, input }) => {
      // Revisar si el producto existe o no
      let producto = Producto.findOne({ id });
    ...
    nuevoPedido: async (_, { input }, ctx) => {
      ...
      let clienteExiste = await Cliente.findOne({ _id: cliente });
      if (!clienteExiste) {
    ...
    eliminarPedido: async (_, { id }, ctx) => {
      // Verificar si el vendedor existe o no
      const pedido = await Pedido.findOne({ _id: id });
      if (!pedido) {
        throw new Error('EL pedido no existe');
      }
